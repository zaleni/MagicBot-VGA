...
def _build_pi05_request(obs_dict, timestep, camera_names):
    """构建官方标准请求格式（匹配 openPI 服务端）。"""
    del timestep, camera_names

    prompt = "Put the desktop trash into the trash bin and put the stationery in the pen holder."
    ...
    def _to_chw_uint8(img):
        ...
        if img_np.ndim == 3 and img_np.shape[-1] == 3:  # HWC -> CHW
            ...
            raise ValueError(f"[PI05] invalid image rank/shape: {img_np.shape}, expected HWC or CHW with 3 channels")

        if img_np.shape != (3, 480, 640):
            raise ValueError(f"[PI05] invalid image shape: {img_np.shape}, expected (3, 480, 640)")

        if img_np.dtype != np.uint8:
            raise ValueError(f"[PI05] invalid image dtype: {img_np.dtype}, expected uint8")
        return img_np
...
def _call_pi05_server(ws_url, timeout_sec, payload):
    """官方客户端调用（自动处理 metadata 握手 + msgpack-numpy 序列化）。"""
    del timeout_sec
    global pi05_client

    if websocket_client_policy is None:
        print("[PI05] openpi_client 不可用，请检查环境安装")
        return None
    ...
    try:
        if pi05_client is None:
            ...
            pi05_client = websocket_client_policy.WebsocketClientPolicy(host=host, port=port)
            try:
                metadata = pi05_client.get_server_metadata()
                print(f"[PI05] 官方客户端连接成功, metadata keys: {list(metadata.keys())}")
            except Exception:
                print("[PI05] 官方客户端连接成功")

        return pi05_client.infer(payload)
    except Exception as e:
        print(f"[PI05] 官方客户端调用失败: {e}")
        # 连接异常时清空单例，下一轮自动重连
        pi05_client = None
        return None
...
def inference_process(args, config, shm_dict, shapes, ros_proc):
    # PI05 WebSocket service config (override by env vars if needed)
    ws_url = os.getenv("PI05_WS_URL", "ws://10.60.43.33:8000")
    ws_timeout = float(os.getenv("PI05_WS_TIMEOUT", "2.0"))
    ...
    print(f"[PI05] WebSocket inference enabled: {ws_url}")
    ...
    while ros_proc.is_alive():
        ...
        while timestep < args.max_publish_step and ros_proc.is_alive():
            ...
            if response is None:
                print("[安全保护] PI05 服务端无返回，本轮跳过并等待下一轮重试。")
                timestep += 1
                continue
            ...
            if first_inference:
                ...
                if not isinstance(response, dict) or response.get("actions", None) is None:
                    print("[安全保护] PI05 返回缺少 actions(50,14)，程序安全退出，不执行机器人动作。")
                    return

                actions_seq = np.asarray(response["actions"], dtype=np.float32)
                if actions_seq.ndim != 2 or actions_seq.shape[1] != 14:
                    print(f"[安全保护] PI05 返回 actions 形状异常: {actions_seq.shape}，期望 (N,14)。程序安全退出。")
                    return

                print("\n" + "=" * 72)
                print("[首次安全确认] 已获取 PI05 动作结果，暂停下发机器人")
                print("=" * 72)
                print("当前机器人14维关节状态 qpos:")
                print("[" + ", ".join([f"{x:8.4f}" for x in qpos]) + "]")

                print("\nPI05 返回动作序列 actions 前30步(每步14维):")
            ...
            if len(action_seq) == 0:
                print("[安全保护] PI05 返回动作为空，本轮跳过并等待下一轮重试。")
                timestep += 1
                continue
