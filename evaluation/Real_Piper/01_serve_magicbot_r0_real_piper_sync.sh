...
MAGICBOT_R0_VIDEO_HEIGHT="${MAGICBOT_R0_VIDEO_HEIGHT:-224}"
MAGICBOT_R0_VIDEO_WIDTH="${MAGICBOT_R0_VIDEO_WIDTH:-448}"
MAGICBOT_R0_STANDARDIZE_VIDEO_SIZE_BY_CAMERAS="${MAGICBOT_R0_STANDARDIZE_VIDEO_SIZE_BY_CAMERAS:-true}"
MAGICBOT_R0_CONCAT_MULTI_CAMERA="${MAGICBOT_R0_CONCAT_MULTI_CAMERA:-horizontal}"
...
if [[ -z "${CHECKPOINT_DIR}" ]]; then
  echo "Please set CHECKPOINT_DIR to a MagicBot_R0 real_piper checkpoint step dir or pretrained_model dir."
  exit 1
fi

case "${RTC_ENABLED,,}" in
  true|1|yes|y|on)
    echo "MagicBot_R0 Real_Piper RTC serving is not implemented. Use sync mode with RTC_ENABLED=false."
    exit 1
    ;;
  false|0|no|n|off)
    ;;
