...
gnome-terminal --title="magicbot-real-lift2" -- $shell_type -i -c "cd '${MAGICBOT_ROOT}'; source ~/.bashrc; conda activate '${RUN_ENV}'; REAL_LIFT2_RUNTIME_ROOT='${REAL_LIFT2_RUNTIME_ROOT}' WS_URL='${WS_URL}' PROMPT='${PROMPT}' FRAME_RATE='${FRAME_RATE}' IMAGE_HISTORY_INTERVAL='${IMAGE_HISTORY_INTERVAL}' SEND_IMAGE_HEIGHT='${SEND_IMAGE_HEIGHT}' SEND_IMAGE_WIDTH='${SEND_IMAGE_WIDTH}' MAX_PUBLISH_STEP='${MAX_PUBLISH_STEP}' RECORD_MODE='${RECORD_MODE}' USE_BASE='${USE_BASE}' FIXED_BODY_HEIGHT='${FIXED_BODY_HEIGHT}' GRIPPER_GATE='${GRIPPER_GATE}' DATA_CONFIG='${DATA_CONFIG}' STATE_DIM='${STATE_DIM}' ACTION_DIM='${ACTION_DIM}' INFERENCE_MODE='${INFERENCE_MODE}' PREFETCH_LEAD_STEPS='${PREFETCH_LEAD_STEPS}' LOG_TIMING_EVERY='${LOG_TIMING_EVERY}' SEED='${SEED}' SAFE_STOP_BODY_HEIGHT='${SAFE_STOP_BODY_HEIGHT}' SAFE_STOP_PUBLISH_STEPS='${SAFE_STOP_PUBLISH_STEPS}' SAFE_STOP_HOME_ARMS='${SAFE_STOP_HOME_ARMS}' SAFE_STOP_HOME_PUBLISH_STEPS='${SAFE_STOP_HOME_PUBLISH_STEPS}' bash '${RUN_SCRIPT}'; $shell_exec"
