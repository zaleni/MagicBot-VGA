...
# CAN
gnome-terminal -t "can1" -x bash -c "cd '${CAN_ROOT}'; ./arx_can1.sh; exec bash;"
sleep 0.3
gnome-terminal -t "can3" -x bash -c "cd '${CAN_ROOT}'; ./arx_can3.sh; exec bash;"
sleep 0.3
gnome-terminal -t "can5" -x bash -c "cd '${CAN_ROOT}'; ./arx_can5.sh; exec bash;"
sleep 0.3

# Body
gnome-terminal --title="body" -x $shell_type -i -c "cd '${BODY_ROOT}'; source install/setup.bash; ros2 launch arx_lift_controller lift.launch.py; $shell_exec"
sleep 1

# Lift
gnome-terminal --title="lift" -x $shell_type -i -c "cd '${X5_ROOT}'; source install/setup.bash; ros2 launch arx_x5_controller v2_joint_control.launch.py; $shell_exec"
sleep 1

# Realsense
gnome-terminal --title="realsense" -x $shell_type -i -c "cd '${REALSENSE_ROOT}'; ./realsense.sh; $shell_exec"
sleep 3

# MagicBot Real_Lift2 inference
gnome-terminal --title="magicbot-real-lift2" -x $shell_type -i -c "cd '${MAGICBOT_ROOT}'; source ~/.bashrc; conda activate '${RUN_ENV}'; WS_URL='${WS_URL}' PROMPT='${PROMPT}' FRAME_RATE='${FRAME_RATE}' IMAGE_HISTORY_INTERVAL='${IMAGE_HISTORY_INTERVAL}' MAX_PUBLISH_STEP='${MAX_PUBLISH_STEP}' RECORD_MODE='${RECORD_MODE}' USE_BASE='${USE_BASE}' FIXED_BODY_HEIGHT='${FIXED_BODY_HEIGHT}' GRIPPER_GATE='${GRIPPER_GATE}' DATA_CONFIG='${DATA_CONFIG}' STATE_DIM='${STATE_DIM}' ACTION_DIM='${ACTION_DIM}' SEED='${SEED}' SAFE_STOP_BODY_HEIGHT='${SAFE_STOP_BODY_HEIGHT}' SAFE_STOP_PUBLISH_STEPS='${SAFE_STOP_PUBLISH_STEPS}' bash '${RUN_SCRIPT}'; $shell_exec"
