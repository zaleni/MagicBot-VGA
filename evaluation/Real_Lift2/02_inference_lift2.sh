...
MAGICBOT_ROOT_Q="$(shell_quote "${MAGICBOT_ROOT}")"
RUN_ENV_Q="$(shell_quote "${RUN_ENV}")"
RUN_SCRIPT_Q="$(shell_quote "${RUN_SCRIPT}")"
MAGICBOT_ENV_ASSIGNMENTS="$(build_env_assignments "${MAGICBOT_ENV_VARS[@]}")"
MAGICBOT_CMD="cd ${MAGICBOT_ROOT_Q}; source ~/.bashrc; conda activate ${RUN_ENV_Q}; env ${MAGICBOT_ENV_ASSIGNMENTS} bash ${RUN_SCRIPT_Q}; ${shell_exec}"
...
gnome-terminal --title="magicbot-real-lift2" -- $shell_type -i -c "${MAGICBOT_CMD}"
