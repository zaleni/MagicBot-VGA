...
if [[ -z "${PRETRAINED_CKPT}" ]]; then
    ...
    echo "Pass the MagicBot_R0 pretrained_model dir, or the checkpoint step dir containing pretrained_model."
    exit 1
fi
...
POLICY_TYPE="${POLICY_TYPE:-MagicBot_R0}"
...
BASE_OUTPUT_PATH="${BASE_OUTPUT_PATH:-${PROJ_ROOT}/evaluation/RoboTwin/output_magicbot_50}"
...
append_optional_magicbot_args() {
    if [[ -n "${WAN_MODEL_ID}" ]]; then
        CMD+=(--args.magicbot_r0_model_id "${WAN_MODEL_ID}")
    fi
    if [[ -n "${WAN_TOKENIZER_MODEL_ID}" ]]; then
        CMD+=(--args.magicbot_r0_tokenizer_model_id "${WAN_TOKENIZER_MODEL_ID}")
    fi
    if [[ -n "${ACTION_DIT_PRETRAINED_PATH}" ]]; then
        CMD+=(--args.magicbot_r0_action_dit_pretrained_path "${ACTION_DIT_PRETRAINED_PATH}")
    fi
    if [[ -n "${FUTURE_3D_PRETRAINED_PATH}" ]]; then
        CMD+=(--args.magicbot_r0_future_3d_pretrained_path "${FUTURE_3D_PRETRAINED_PATH}")
    fi
    if [[ -n "${MAGICBOT_R0_STATS_PATH}" ]]; then
        CMD+=(--args.magicbot_r0_stats_path "${MAGICBOT_R0_STATS_PATH}")
    fi
...
build_cmd() {
    ...
    CMD=(
        ...
        --args.magicbot_r0_video_height "${VIDEO_HEIGHT}"
        --args.magicbot_r0_video_width "${VIDEO_WIDTH}"
        --args.magicbot_r0_concat_multi_camera "${CONCAT_MULTI_CAMERA}"
    )
    append_optional_magicbot_args
}
...
echo "Launching MagicBot_R0 RoboTwin randomized evaluation:"
