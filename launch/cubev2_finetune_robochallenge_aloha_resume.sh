...
# Defaults follow launch/cubev2_finetune_real_lift2_abs.sh on the jiangjiahao server.
...
#   ROBOCHALLENGE_ALOHA_EXTRA_TASKS="" bash launch/cubev2_finetune_robochallenge_aloha.sh
...
PY
  ...
  if [[ "${USE_EXTERNAL_STATS}" == "true" && ! -f "${DATASET_EXTERNAL_STATS_PATH}" ]]; then
    ...
    echo "Compute them first with: ACTION_TYPE=${ACTION_TYPE} ROBOCHALLENGE_ROOT=... bash launch/compute_norm_stats_robochallenge_aloha.sh"
    exit 1
  fi
