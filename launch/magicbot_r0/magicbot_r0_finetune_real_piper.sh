...
TRAIN_MAX_STEPS="${TRAIN_MAX_STEPS:-30000}"
...
if [[ "${USE_EXTERNAL_STATS}" == "true" && ! -f "${NORMALIZATION_STATS_PATH}" ]]; then
  ...
  echo "  DATASET_DIR=\"${DATASET_DIR}\" ACTION_TYPE=\"${ACTION_TYPE}\" CHUNK_SIZE=\"${ACTION_HORIZON}\" NORM_STATS_ROOT=\"${NORM_STATS_ROOT}\" bash launch/compute_norm/compute_norm_stats_real_piper_delta.sh"
  exit 1
fi
...
if missing:
    raise SystemExit(f"Missing required MagicBot_R0 Real_Piper features: {missing}")
...
JOB_NAME="${JOB_NAME:-${POLICY}-${MAGICBOT_R0_VARIANT}-real_piper-${DATASET_NAME}-${ACTION_TYPE}-${BOOTSTRAP_TAG}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
...
if [[ "${LOAD_TEXT_ENCODER}" == "true" ]]; then
  ...
  TEXT_EMBED_CACHE_DIR="${TEXT_EMBED_CACHE_DIR:-${PROJ_ROOT}/outputs/MagicBot_R0/text_embeds/real_piper/${DATASET_NAME}}"
fi
...
ARGS=(
    ...
    --wandb.enable=true
    --wandb.project=MagicBot_R0_RealPiper
