...
DATASET_DIR="${DATASET_DIR:-/inspire/ssd/project/embodied-basic-model/zhangjianing-253108140206/DATASET/zhenji/lerobot_v30/sweeping_100_filter_60hz_v30}"
...
USE_RAMDISK_DATASET="${USE_RAMDISK_DATASET:-false}"
...
NORM_STATS_ROOT="${NORM_STATS_ROOT:-/inspire/ssd/project/embodied-basic-model/zhangjianing-253108140206/DATASET/zhenji/norm_stats_48}"
...
if [[ "${USE_EXTERNAL_STATS}" == "true" && ! -f "${NORMALIZATION_STATS_PATH}" ]]; then
  ...
  echo "  DATASET_DIR=\"${DATASET_DIR}\" ACTION_TYPE=\"${ACTION_TYPE}\" CHUNK_SIZE=\"${ACTION_HORIZON}\" NORM_STATS_ROOT=\"${NORM_STATS_ROOT}\" bash launch/compute_norm/compute_norm_stats_real_lift2_delta.sh"
  exit 1
fi
...
if missing:
    raise SystemExit(f"Missing required MagicBot_R0 Real_Lift2 features: {missing}")
' "${DATASET_DIR}/meta/info.json" "${IMAGE_KEYS}"
else
  echo "Skipping per-dataset validation (VALIDATE_DATASETS=${VALIDATE_DATASETS})."
fi

BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-/inspire/qb-ilm/project/embodied-basic-model/zhangjianing-253108140206/outputs_real/${POLICY}}"
...
if [[ "${LOAD_TEXT_ENCODER}" == "true" ]]; then
  ...
  TEXT_EMBED_CACHE_DIR="${TEXT_EMBED_CACHE_DIR:-${PROJ_ROOT}/outputs/MagicBot_R0/text_embeds/real_lift2/${DATASET_NAME}}"
fi
