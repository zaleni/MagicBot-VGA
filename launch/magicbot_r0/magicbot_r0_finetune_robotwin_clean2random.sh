...
# MagicBot-R0 RoboTwin clean->randomized (clean2random) finetuning launcher.
#
# Same clean2random recipe as the WSA_Large launcher
# (WSA_Dev/launch/WSA_Large/wsa_large_finetune_robotwin_clean2random.sh),
# adapted to MagicBot_R0's native finetune spec
# (launch/magicbot_r0/magicbot_r0_finetune_robotwin_delta.sh):
#   delta / 24D padded actions (14-entry delta mask), lr 1e-4, warmup 2000,
#   decay lr 1e-6, batch 12, seed 42, bf16, pyav, mixed_attn=false,
#   init = the 300k multidata-delta pretrained checkpoint below.
# Data is restricted to the clean RoboTwin subset: 50 tasks x 50 episodes
# (aloha-agilex_clean_50), i.e. 2,500 clean episodes and no randomized demos.
# Duration follows the clean2random shared protocol: 60k optimizer steps
# (TRAIN_MAX_STEPS), final-checkpoint selection, saves are recovery-only.
#
# Stage 2 (randomized-data finetune) can reuse this script with
# ROBOTWIN_DATASET_VARIANT=aloha-agilex_randomized_500 RESUME=true
# RESUME_CHECKPOINT_DIR=<stage-1 checkpoints/<step>> TRAIN_MAX_STEPS=<new TOTAL>
# ROBOTWIN_EXPECTED_EPISODES=25000   (still 50 tasks, 500 eps each; 0=off).
# NOTE: TRAIN_MAX_STEPS is the TOTAL step budget for the run, not an additional
# count — a stage-1 run stopped at 60000 must be resumed with e.g. 120000,
# otherwise the trainer restores step=60000 >= cfg.steps and exits immediately.

###############################################################################
################################# ENV config ##################################

export MASTER_ADDR=${MASTER_ADDR:-"127.0.0.1"}
export MASTER_PORT=${MASTER_PORT:-7350}
...
# Stage-1 init: the MagicBot-R0 pretrained model (300k multidata-delta
# pretrain), kept identical to magicbot_r0_finetune_robotwin_delta.sh.
POLICY_INIT_PATH="${POLICY_INIT_PATH:-${PRETRAINED_PATH:-${PRETRAINED_MODEL_DIR:-/inspire/ssd/project/embodied-basic-model/zhangjianing-253108140206/MagicBot-VGA/outputs/MagicBot_R0/MagicBot_R0-magicbot_r0-multidata-delta-pretrain-2026_05_07_15_59_20/checkpoints/300000/pretrained_model}}}"
...
# Clean-subset selector: keep only datasets whose directory basename matches
# this glob. "aloha-agilex_clean_50" = 50 tasks x 50 clean episodes (2,500).
# Set it to "aloha-agilex_randomized_500" for the randomized stage, or to an
# empty value (ROBOTWIN_DATASET_VARIANT=) to disable filtering entirely.
ROBOTWIN_DATASET_VARIANT="${ROBOTWIN_DATASET_VARIANT-aloha-agilex_clean_50}"
ROBOTWIN_EXPECTED_TASKS="${ROBOTWIN_EXPECTED_TASKS:-50}"
ROBOTWIN_EXPECTED_EPISODES="${ROBOTWIN_EXPECTED_EPISODES:-2500}"
case "${ROBOTWIN_EXPECTED_TASKS}" in ''|*[!0-9]*) echo "ROBOTWIN_EXPECTED_TASKS must be a non-negative integer (got '${ROBOTWIN_EXPECTED_TASKS}')."; exit 1 ;; esac
case "${ROBOTWIN_EXPECTED_EPISODES}" in ''|*[!0-9]*) echo "ROBOTWIN_EXPECTED_EPISODES must be a non-negative integer (got '${ROBOTWIN_EXPECTED_EPISODES}')."; exit 1 ;; esac
# Force base-10 so values like "08" or "050" are not read as octal.
ROBOTWIN_EXPECTED_TASKS=$((10#${ROBOTWIN_EXPECTED_TASKS}))
ROBOTWIN_EXPECTED_EPISODES=$((10#${ROBOTWIN_EXPECTED_EPISODES}))
ROBOTWIN_CHECK_EPISODES="${ROBOTWIN_CHECK_EPISODES:-true}"
...
# Clean-50-only mean/std statistics, shared across models in the clean2random
# comparison. When unset the script falls back to the full-RoboTwin
# norm_stats_32 file (the base finetune's value) and warns.
CLEAN_STATS_PATH="${CLEAN_STATS_PATH:-}"
VALIDATE_DATASETS="${VALIDATE_DATASETS:-true}"
VIDEO_BACKEND="${VIDEO_BACKEND:-pyav}"
USE_DIST_LOADING="${USE_DIST_LOADING:-false}"

# Same action spec as the base MagicBot_R0 RoboTwin finetune: delta / 24D.
...
STEPS="${STEPS:-}"
# clean2random shared protocol: 60k optimizer steps, final-checkpoint selection.
TRAIN_MAX_STEPS="${TRAIN_MAX_STEPS:-60000}"
...
export LEROBOT_MAGICBOT_R0_EVAL_NUM_WORKERS="${EVAL_NUM_WORKERS}"

# Base recipe: lr 1e-4, cosine to 1e-6 (the base finetune used warmup 5000;
# the default here is 2000 like the WSA_Large clean2random launcher — set
# WARMUP_STEPS=5000 to match the base recipe exactly).
LR="${LR:-1.0e-4}"
...
if [[ "${USE_EXTERNAL_STATS}" == "true" ]]; then
  ...
  if [[ -n "${CLEAN_STATS_PATH}" ]]; then
    NORMALIZATION_STATS_PATH="${CLEAN_STATS_PATH}"
  elif [[ -n "${NORMALIZATION_STATS_PATH}" ]]; then
    echo "[note] CLEAN_STATS_PATH is not set — using ${NORMALIZATION_STATS_PATH}."
    echo "[note] That file holds full-RoboTwin statistics, NOT clean-50-only ones."
    echo "[note] For the strict clean2random contract, copy the clean-50 delta stats"
    echo "[note] (WSA_Dev repo: exp_material/clean2random_external_baselines/manifests/robotwin_clean50_delta_stats.json)"
    echo "[note] to the server and set CLEAN_STATS_PATH=<path>."
  fi

  if [[ -z "${NORMALIZATION_STATS_PATH}" ]]; then
    echo "USE_EXTERNAL_STATS=true but no normalization stats path could be resolved."
    echo "Set NORMALIZATION_STATS_PATH, DATASET_EXTERNAL_STATS_PATH, DATASET_EXTERNAL_STATS_ROOT, or CLEAN_STATS_PATH."
    exit 1
  fi
else
...
discover_dataset_dirs() {
  ...
  find -L "${root}" -path "*/meta/info.json" 2>/dev/null \
    | while read -r info_path; do
        ...
        if [[ -n "${ROBOTWIN_DATASET_VARIANT}" ]]; then
          # shellcheck disable=SC2053  # intentional glob match on the right-hand side
          if [[ "$(basename "${ds_dir}")" != ${ROBOTWIN_DATASET_VARIANT} ]]; then
            continue
          fi
        fi
...
if [[ ${#DATASET_REPO_IDS[@]} -eq 0 ]]; then
  ...
  if [[ -n "${ROBOTWIN_DATASET_VARIANT}" ]]; then
    echo "The variant filter kept only directories named '${ROBOTWIN_DATASET_VARIANT}'."
    echo "Set ROBOTWIN_DATASET_VARIANT= (empty) to disable filtering, or check the dataset layout."
  fi
  if [[ "${ROBOTWIN_REQUIRE_THREE_CAMERAS}" == "true" ]]; then
...
if [[ ${ROBOTWIN_EXPECTED_TASKS} -gt 0 && ${#DATASET_REPO_IDS[@]} -ne ${ROBOTWIN_EXPECTED_TASKS} ]]; then
  echo "[warn] Found ${#DATASET_REPO_IDS[@]} '${ROBOTWIN_DATASET_VARIANT}' datasets, expected ${ROBOTWIN_EXPECTED_TASKS}."
  echo "[warn] Continuing with the discovered set — set ROBOTWIN_EXPECTED_TASKS=0 to silence this warning."
fi
...
if [[ "${ROBOTWIN_CHECK_EPISODES}" == "true" && ${#DATASET_REPO_IDS[@]} -gt 0 ]]; then
  TOTAL_EPISODES="$(
    python -c 'import json, sys
total = 0
for ds_dir in sys.argv[1:]:
    try:
        total += int(json.load(open(ds_dir + "/meta/info.json", encoding="utf-8")).get("total_episodes", 0) or 0)
    except Exception:
        pass
print(total)
' "${DATASET_REPO_IDS[@]}"
  )"
  echo "Total episodes across selected datasets: ${TOTAL_EPISODES}"
  if [[ "${ROBOTWIN_EXPECTED_EPISODES}" -gt 0 && "${TOTAL_EPISODES}" != "${ROBOTWIN_EXPECTED_EPISODES}" ]]; then
    echo "[warn] Episode total ${TOTAL_EPISODES} != expected ${ROBOTWIN_EXPECTED_EPISODES} clean episodes."
    echo "[warn] Continuing — set ROBOTWIN_EXPECTED_EPISODES=0 to silence this warning."
  fi
fi
...
CLEAN2RANDOM_TAG="${CLEAN2RANDOM_TAG:-clean50}"
if [[ "${RESUME}" == "true" ]]; then
  JOB_NAME="${JOB_NAME:-${RESUME_CONFIG_JOB_NAME:-${MAGICBOT_R0_VARIANT}-robotwin-clean2random-${ACTION_TYPE}-${CLEAN2RANDOM_TAG}-resume}}"
  OUTPUT_DIR="${RESUME_OUTPUT_DIR}"
  REPO_ID_FILE_DIR="${OUTPUT_DIR}/_repo_id_files"
else
  JOB_NAME="${JOB_NAME:-${MAGICBOT_R0_VARIANT}-robotwin-clean2random-${ACTION_TYPE}-${CLEAN2RANDOM_TAG}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
...
echo "DATASET_VARIANT_FILTER=${ROBOTWIN_DATASET_VARIANT}"
echo "TRAIN_MAX_STEPS=${TRAIN_MAX_STEPS:-<disabled>}, NUM_EPOCHS=${NUM_EPOCHS:-<disabled>}, STEPS=${STEPS:-<unset>}"
...
echo "TEXT_EMBED_CACHE_MAX_ENTRIES=${TEXT_EMBED_CACHE_MAX_ENTRIES}"
...
echo "EVAL_FREQ=${EVAL_FREQ}, EVAL_NUM_WORKERS=${EVAL_NUM_WORKERS}"
...
if [[ "${DRY_RUN:-false}" == "true" ]]; then
    echo "[dry-run] accelerate launch has not been invoked. Full command:"
    printf ' %q' "${ARGS[@]}"
    echo
    exit 0
fi
