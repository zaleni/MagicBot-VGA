...
# group_frames_pow gives regular/extra fixed group budgets, then allocates
# each group by task frame_count^TASK_SAMPLING_GAMMA.
...
DATASET_EXTERNAL_STATS_PATH="${DATASET_EXTERNAL_STATS_PATH:-${NORM_STATS_ROOT}/robochallenge_raw_w1/${TASK_PRESET}/${ACTION_TYPE}/chunk${CHUNK_SIZE}_frame${FRAME_INTERVAL}/stats.json}"
...
if [[ "${USE_EXTERNAL_STATS}" != "true" ]]; then
  echo "RoboChallenge raw W1 training requires USE_EXTERNAL_STATS=true."
  exit 1
fi
...
if [[ ! -f "${DATASET_EXTERNAL_STATS_PATH}" ]]; then
  ...
  echo "  ROBOCHALLENGE_RAW_ROOT=... TASK_PRESET=${TASK_PRESET} ACTION_TYPE=${ACTION_TYPE} CHUNK_SIZE=${CHUNK_SIZE} FRAME_INTERVAL=${FRAME_INTERVAL} bash launch/compute_norm/compute_norm_stats_robochallenge_raw_w1.sh"
  exit 1
fi

BASE_OUTPUT_DIR="${BASE_OUTPUT_DIR:-outputs_robochallenge/${POLICY}}"
JOB_NAME="${JOB_NAME:-${POLICY}-robochallenge_raw_w1-${TASK_PRESET}-${ACTION_TYPE}-chunk${CHUNK_SIZE}-frame${FRAME_INTERVAL}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
...
ARGS=(
    ...
    --dataset.type=robochallenge_raw_w1
    --dataset.repo_id=robochallenge_raw_w1
    --dataset.raw_root="${ROBOCHALLENGE_RAW_ROOT}"
    --dataset.embodiment=DOS-W1
