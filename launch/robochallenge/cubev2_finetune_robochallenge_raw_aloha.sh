...
if [[ ! -f "${DATASET_EXTERNAL_STATS_PATH}" ]]; then
  ...
  echo "  ROBOCHALLENGE_RAW_ROOT=... TASK_PRESET=${TASK_PRESET} ACTION_TYPE=${ACTION_TYPE} CHUNK_SIZE=${CHUNK_SIZE} FRAME_INTERVAL=${FRAME_INTERVAL} bash launch/compute_norm/compute_norm_stats_robochallenge_raw_aloha.sh"
  exit 1
fi
