...
POLICY="cubev2"
OLD_CHECKPOINT_DIR="${OLD_CHECKPOINT_DIR:-outputs_robochallenge/cubev2/cubev2-robochallenge_aloha-regular_only-delta-chunk50-finetune-2026_05_10_02_42_28/checkpoints/050000/pretrained_model}"
OLD_PRETRAINED_MODEL_DIR="${OLD_PRETRAINED_MODEL_DIR:-${PRETRAINED_MODEL_DIR:-}}"
POLICY_INIT_PATH="${POLICY_INIT_PATH:-${PRETRAINED_PATH:-}}"

if [[ -z "${POLICY_INIT_PATH}" && -n "${OLD_PRETRAINED_MODEL_DIR}" ]]; then
  POLICY_INIT_PATH="${OLD_PRETRAINED_MODEL_DIR%/}"
fi

if [[ -z "${POLICY_INIT_PATH}" && -n "${OLD_CHECKPOINT_DIR}" ]]; then
  if [[ -f "${OLD_CHECKPOINT_DIR%/}/pretrained_model/config.json" ]]; then
    POLICY_INIT_PATH="${OLD_CHECKPOINT_DIR%/}/pretrained_model"
  elif [[ -f "${OLD_CHECKPOINT_DIR%/}/config.json" ]]; then
    POLICY_INIT_PATH="${OLD_CHECKPOINT_DIR%/}"
  else
    echo "Could not find pretrained_model/config.json or config.json under OLD_CHECKPOINT_DIR=${OLD_CHECKPOINT_DIR}"
    exit 1
  fi
fi

if [[ -z "${POLICY_INIT_PATH}" ]]; then
  echo "Set OLD_CHECKPOINT_DIR, OLD_PRETRAINED_MODEL_DIR, POLICY_INIT_PATH, or PRETRAINED_PATH."
  echo "Preferred: OLD_CHECKPOINT_DIR=/path/to/old_run/checkpoints/50000/pretrained_model"
  exit 1
fi

if [[ ! -f "${POLICY_INIT_PATH%/}/config.json" ]]; then
  echo "Policy config not found: ${POLICY_INIT_PATH%/}/config.json"
  exit 1
fi
...
# Default to the regular-only task set used by the previous run.
...
OPTIMIZER_LR="${OPTIMIZER_LR:-${LR:-3.2e-5}}"
SCHEDULER_WARMUP_STEPS="${SCHEDULER_WARMUP_STEPS:-${WARMUP_STEPS:-1200}}"
SCHEDULER_DECAY_STEPS="${SCHEDULER_DECAY_STEPS:-${DECAY_STEPS:-${STEPS}}}"
SCHEDULER_DECAY_LR="${SCHEDULER_DECAY_LR:-${DECAY_LR:-3e-6}}"
SEED="${SEED:-242}"
...
JOB_NAME="${JOB_NAME:-${POLICY}-robochallenge_aloha${JOB_TASK_SUFFIX}-${ACTION_TYPE}-chunk${CHUNK_SIZE}-restart-bs${EFFECTIVE_BATCH_SIZE}-$(date +'%Y_%m_%d_%H_%M_%S')}"
...
if [[ "${USE_EXTERNAL_STATS}" == "true" && ! -f "${DATASET_EXTERNAL_STATS_PATH}" ]]; then
  ...
  echo "Compute them first with: ACTION_TYPE=${ACTION_TYPE} CHUNK_SIZE=${CHUNK_SIZE} ROBOCHALLENGE_ROOT=... bash launch/compute_norm/compute_norm_stats_robochallenge_aloha.sh"
  exit 1
fi
...
echo "OLD_CHECKPOINT_DIR=${OLD_CHECKPOINT_DIR:-<unset>}"
...
echo "NOTE: This is not true resume. Optimizer, scheduler, step, and RNG start fresh."
printf '  - %s\n' "${DATASET_REPO_IDS[@]}"
