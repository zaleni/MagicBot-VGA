...
if [[ "${RESUME}" == "true" ]]; then
  ...
  if [[ "${robot_type}" != "real_piper" ]]; then
    ...
    echo "Please reconvert the dataset with --robot-type real_piper or update meta/info.json."
    exit 1
  fi
  ...
  if [[ "${USE_EXTERNAL_STATS}" == "true" && ! -f "${DATASET_EXTERNAL_STATS_PATH}" ]]; then
    ...
    echo "  DATASET_DIR=\"${DATASET_DIR}\" ACTION_TYPE=\"${ACTION_TYPE}\" CHUNK_SIZE=\"${CHUNK_SIZE}\" NORM_STATS_ROOT=\"${NORM_STATS_ROOT}\" bash launch/compute_norm/compute_norm_stats_real_piper_delta.sh"
    exit 1
  fi
  ...
  if [[ -z "${JOB_NAME}" ]]; then
    JOB_NAME="${POLICY}-real_piper-${DATASET_NAME}-${ACTION_TYPE}-chunk${CHUNK_SIZE}-${INIT_TAG}-${CUBEV2_ATTENTION_MASK_MODE}-gen${LAMBDA_GEN}-3d${LAMBDA_3D}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')"
  fi
...
if [[ "${RESUME}" == "true" ]]; then
    ...
    ARGS+=(
        ...
        --wandb.enable=true
        --wandb.project=CUBEv2_RealPiper
