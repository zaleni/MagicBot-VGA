...
if [[ "${robot_type}" != "real_piper" ]]; then
  ...
  echo "Please reconvert the dataset with --input-schema real_piper or update meta/info.json."
  exit 1
fi
...
JOB_NAME="${POLICY}-real_piper-${DATASET_NAME}-${ACTION_TYPE}-chunk${CHUNK_SIZE}-attn-${CUBEV2_ATTENTION_MASK_MODE}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')"
