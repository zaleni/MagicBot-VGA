...
DATASET_EXTERNAL_STATS_ROOT="${DATASET_EXTERNAL_STATS_ROOT:-/inspire/ssd/project/embodied-basic-model/zhangjianing-253108140206/MagicBot-VGA/norm_stats_32}"
if [[ -z "${DATASET_EXTERNAL_STATS_PATH}" && -z "${DATASET_EXTERNAL_STATS_ROOT}" ]]; then
  DATASET_EXTERNAL_STATS_PATH="/inspire/ssd/project/embodied-basic-model/zhangjianing-253108140206/Foundation-Moodel/norm_stats/aloha/${ACTION_TYPE}/stats.json"
fi
...
if [[ "${CUBEV2_ATTENTION_MASK_MODE}" == "causal" && "${ENABLE_3D_QUERIES}" != "true" ]]; then
  echo "CUBEV2_ATTENTION_MASK_MODE=causal requires ENABLE_3D_QUERIES=true."
  exit 1
fi
...
JOB_NAME="${JOB_NAME:-${POLICY}-robotwin-${ACTION_TYPE}-chunk${CHUNK_SIZE}-${INIT_TAG}-${CUBEV2_ATTENTION_MASK_MODE}-gen${LAMBDA_GEN}-3d${LAMBDA_3D}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
