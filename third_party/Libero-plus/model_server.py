...
class MagicBotR0LiberoPolicy:
    def __init__(self, args: ServeArgs):
        ...
        if self.action_mode != "abs":
            raise NotImplementedError("MagicBot_R0 LIBERO-plus serving currently expects abs action mode.")
        ...
        if not self.load_text_encoder and not self.text_embed_cache_dir.is_dir():
            raise FileNotFoundError(
                "MagicBot_R0 serving defaults to cached LIBERO-plus text embeddings, but "
    ...
    def _resolve_text_embed_cache_dir(self) -> Path:
        ...
        return REPO_ROOT / "outputs" / "MagicBot_R0" / "text_embeds" / "libero_plus"
    ...
    def _load_cached_text_context(self, prompt: str) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if not cache_path.is_file():
            raise FileNotFoundError(
                ...
                "Either precompute the cache under `outputs/MagicBot_R0/text_embeds/libero_plus`, "
                "or set LOAD_TEXT_ENCODER=true to encode prompts on the fly."
            )
...
def main(args: ServeArgs) -> None:
    ...
    if config.type == "cubev2":
        ...
        raise ValueError(f"Unsupported LIBERO-plus checkpoint type: {config.type!r}")
