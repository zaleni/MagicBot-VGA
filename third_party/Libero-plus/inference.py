...
LIBERO_ACTION_DIM = 7
LIBERO_PLUS_DEFAULT_TASK_SUITES = ("libero_spatial", "libero_object", "libero_goal", "libero_10")
...
class EvalArgs:
    ...
    task_suite_name: str = ""
    task_suite_names: str = ",".join(LIBERO_PLUS_DEFAULT_TASK_SUITES)
    ...
    video_dir: Path = ROOT_PATH / "evaluation" / "libero-plus" / "output"
    fps: int = 10
...
def ensure_libero_available() -> None:
    if benchmark is None or get_libero_path is None or OffScreenRenderEnv is None:
        ...
        raise ImportError(
            "LIBERO/LIBERO-plus is not available in the current environment. "
            "Please install LIBERO-plus and its simulator dependencies before running evaluation. "
            f"Python executable: {sys.executable}. "
            f"Search roots checked: {search_hint or '(none)'}. "
            "If LIBERO-plus is already checked out locally, set LIBERO_HOME=/path/to/LIBERO-plus "
...
def resolve_ckpt_dir(ckpt_path: Union[str, Path]) -> Path:
    ...
    if not ckpt_str.strip():
        raise ValueError(
            "ckpt_path is required for local LIBERO-plus evaluation. "
...
def get_max_steps(task_suite_name: str) -> int:
    ...
    if task_suite_name == "libero_90":
        return 400
    raise ValueError(f"Unknown LIBERO-plus task suite: {task_suite_name}")
...
def make_task_dir_name(task_id: int, task_description: str, max_name_chars: int = 96) -> str:
    task_name = sanitize_name(task_description) or f"task_{task_id:02d}"
    if len(task_name) > max_name_chars:
        digest = hashlib.sha1(task_description.encode("utf-8")).hexdigest()[:10]
        task_name = f"{task_name[:max_name_chars].rstrip('_')}_{digest}"
    return f"{task_id:02d}_{task_name}"


def resolve_task_suite_names(args: EvalArgs) -> list[str]:
    if args.task_suite_name.strip():
        suite_names = [args.task_suite_name.strip()]
    else:
        suite_names = [part.strip() for part in args.task_suite_names.split(",") if part.strip()]

    if not suite_names:
        raise ValueError("No LIBERO-plus task suites were provided.")
    ...
    for suite_name in suite_names:
        if suite_name in seen:
            continue
        seen.add(suite_name)
        deduped.append(suite_name)
    return deduped


def _get_task_classification_candidates(explicit_path: str) -> list[Path]:
    candidates: list[Path] = []

    if explicit_path.strip():
        candidates.append(Path(explicit_path).expanduser())

    env_path = os.environ.get("LIBERO_PLUS_CLASSIFICATION_PATH", "").strip()
    if env_path:
    ...
    for search_root in LIBERO_SEARCH_ROOTS:
        candidates.extend(
            [
                search_root / "libero" / "libero" / "benchmark" / "task_classification.json",
                search_root / "libero" / "benchmark" / "task_classification.json",
                search_root / "benchmark" / "task_classification.json",
            ]
        )

    libero_module = sys.modules.get("libero")
    libero_file = getattr(libero_module, "__file__", None)
    if libero_file:
        libero_root = Path(libero_file).resolve().parent
        candidates.extend(
            [
                libero_root / "benchmark" / "task_classification.json",
                libero_root.parent / "benchmark" / "task_classification.json",
            ]
        )
...
def _normalize_task_classification(payload: Any) -> dict[int, dict[str, Any]]:
    ...
    for task_id, metadata in payload.items():
        try:
        ...
        if isinstance(metadata, dict):
            normalized[normalized_task_id] = metadata
        else:
            normalized[normalized_task_id] = {"value": metadata}
    return normalized


def load_task_classification(explicit_path: str) -> tuple[dict[int, dict[str, Any]], Path | None]:
    for candidate in _get_task_classification_candidates(explicit_path):
        if not candidate.is_file():
            continue
        with candidate.open("r", encoding="utf-8") as handle:
            payload = json.load(handle)
        return _normalize_task_classification(payload), candidate
    return {}, None


def _extract_classification_label(metadata: dict[str, Any], keys: tuple[str, ...]) -> str | None:
    for key in keys:
...
def build_classification_summary(task_summaries: list[dict[str, Any]]) -> dict[str, dict[str, dict[str, Any]]]:
    category_totals: dict[str, dict[str, int]] = defaultdict(lambda: {"successes": 0, "episodes": 0, "tasks": 0})
    difficulty_totals: dict[str, dict[str, int]] = defaultdict(lambda: {"successes": 0, "episodes": 0, "tasks": 0})

    for task_summary in task_summaries:
        metadata = task_summary.get("classification")
        if not isinstance(metadata, dict):
            continue

        category = _extract_classification_label(
            metadata,
            ("category", "perturbation_category", "perturbation", "dimension", "group"),
        )
        difficulty = _extract_classification_label(
            metadata,
            ("difficulty", "difficulty_level", "difficulty_tier", "level"),
        )

        successes = int(task_summary["successes"])
        episodes = int(task_summary["num_trials"])

        if category is not None:
            category_totals[category]["successes"] += successes
            category_totals[category]["episodes"] += episodes
            category_totals[category]["tasks"] += 1
        if difficulty is not None:
            difficulty_totals[difficulty]["successes"] += successes
            difficulty_totals[difficulty]["episodes"] += episodes
            difficulty_totals[difficulty]["tasks"] += 1

    def _finalize(summary: dict[str, dict[str, int]]) -> dict[str, dict[str, Any]]:
        finalized: dict[str, dict[str, Any]] = {}
        for label, totals in sorted(summary.items()):
            episodes = max(totals["episodes"], 1)
            finalized[label] = {
                "successes": int(totals["successes"]),
                "total_episodes": int(totals["episodes"]),
                "task_count": int(totals["tasks"]),
                "success_rate": float(totals["successes"] / episodes),
            }
        return finalized

    return {
        "by_category": _finalize(category_totals),
        "by_difficulty": _finalize(difficulty_totals),
    }
...
def build_policy_and_transforms(args: EvalArgs):
    ...
    if config.type != "cubev2":
        raise ValueError(f"LIBERO-plus evaluation currently supports CubeV2 checkpoints only, got {config.type!r}.")
    ...
    if processor_path is None:
        raise ValueError("Failed to resolve a Qwen3-VL processor path for CubeV2 LIBERO-plus evaluation.")
...
def build_remote_policy_client(args: EvalArgs):
    try:
        from .libero_remote_client import LiberoRemoteClient
    except ImportError:
        from libero_remote_client import LiberoRemoteClient
...
def evaluate_suite(args: EvalArgs) -> None:
    ...
    task_suite_names = resolve_task_suite_names(args)

    task_classification, task_classification_path = load_task_classification(args.classification_path)
    if task_classification_path is not None:
        logging.info(
            "Loaded LIBERO-plus task classification from %s (%s entries).",
            task_classification_path,
            len(task_classification),
        )
    else:
        logging.info("No LIBERO-plus task classification file was found; category summaries will be skipped.")
    ...
    try:
        ...
        logging.info(
            "Starting LIBERO-plus evaluation for suites=%s, task_id=%s. Results will be saved to %s",
        ...
        for task_suite_name in task_suite_names:
            if task_suite_name not in benchmark_dict:
                ...
                raise ValueError(
                    f"Unknown LIBERO-plus task suite: {task_suite_name}. Available suites: {available}"
                )
            ...
            if args.task_id is not None:
                task_indices = list(range(args.task_id, task_suite.n_tasks))
            else:
                task_indices = list(range(task_suite.n_tasks))

            suite_video_dir = args.video_dir / task_suite_name
            suite_video_dir.mkdir(parents=True, exist_ok=True)
            logging.info(
                "Evaluating LIBERO-plus suite=%s with %s reported tasks. Results will be saved to %s",
                task_suite_name,
                task_suite.n_tasks,
                suite_video_dir,
            )
            logging.info(
                "Selected task ids for suite=%s: %s",
                task_suite_name,
                f"{task_indices[0]}..{task_indices[-1]}" if len(task_indices) > 1 else str(task_indices[0]),
            )

            suite_task_summaries: list[dict[str, Any]] = []
            ...
            for task_id in task_indices:
                ...
                if num_trials < args.num_trials_per_task:
                    logging.warning(
                        "Suite %s task %s only provides %s initial states; clamping num_trials_per_task from %s to %s.",
                        task_suite_name,
                        task_id,
                        len(initial_states),
                ...
                try:
                    task_name = sanitize_name(task_description) or f"task_{task_id:02d}"
                    task_dir = suite_video_dir / make_task_dir_name(task_id, task_description)
                    ...
                    for episode_idx in range(num_trials):
                        ...
                        logging.info(
                            "[suite=%s task=%02d (%s) episode=%03d] %s | suite success rate: %.2f%% | overall success rate: %.2f%%",
                            task_suite_name,
                            task_id,
                            task_name,
                            episode_idx,
                            suffix,
                            100.0 * suite_total_successes / max(suite_total_episodes, 1),
                ...
                classification = task_classification.get(int(task_id))
                if classification is not None:
                    task_summary["classification"] = classification
                task_summaries.append(task_summary)
                suite_task_summaries.append(task_summary)
            ...
            suite_summary = {
                ...
                "seed": int(args.seed),
                "reported_task_count": int(task_suite.n_tasks),
                "evaluated_task_count": int(len(task_indices)),
                "total_episodes": int(suite_total_episodes),
                "total_successes": int(suite_total_successes),
                "success_rate": float(suite_total_successes / max(suite_total_episodes, 1)),
                "task_summaries": suite_task_summaries,
            }
            suite_summary_path = suite_video_dir / "summary.json"
            write_json(suite_summary_path, suite_summary)
            suite_summaries.append(suite_summary)
            logging.info(
                "Finished suite=%s. Success rate: %.2f%% (%s/%s). Summary saved to %s",
                task_suite_name,
                100.0 * suite_summary["success_rate"],
                suite_total_successes,
                suite_total_episodes,
                suite_summary_path,
            )

        overall_summary = {
            "task_suite_name": args.task_suite_name or None,
        ...
        if task_classification_path is not None:
            overall_summary["classification_path"] = str(task_classification_path)
        classification_summary = build_classification_summary(task_summaries)
        if classification_summary["by_category"] or classification_summary["by_difficulty"]:
            overall_summary["classification_summary"] = classification_summary
        ...
        logging.info(
            "Finished LIBERO-plus evaluation for suites=%s task_id=%s (%s). Success rate: %.2f%% (%s/%s). Summary saved to %s",
            ",".join(task_suite_names),
            args.task_id if args.task_id is not None else "all",
